...
export const getRebillV3Initialization = (country: string) => {
	...
	switch (country) {
		...
		case 'uy':
			return IS_PROD ? NEXT_PUBLIC_REBILL_UY_PK_PRD : NEXT_PUBLIC_REBILL_UY_PK_TEST;
		case 'mx':
			return IS_PROD ? NEXT_PUBLIC_REBILL_MX_PK_PRD : NEXT_PUBLIC_REBILL_MX_PK_TEST;
		default:
			//return te old rebill DEPRECATE_REBILL
			return IS_PROD ? NEXT_PUBLIC_REBILL_DEPRECATED_PK_PRD : NEXT_PUBLIC_REBILL_DEPRECATED_PK_TEST;
	}
};

const mappingCheckoutFields = (contactZoho: ContactCRM) => {
	const { area_code, number } = separatePhoneNumber(contactZoho.Phone);
	const parsedIdentification = contactZoho.Identificacion.replace(/[\u2010-\u2015\u2212\uFE58\uFE63\uFF0D]/g, '');
	const mapping = {
		customerInformation: {
	...
	console.log({ mapping, parsedIdentification }, typeof contactZoho.Identificacion, contactZoho.Identificacion);
...
const getPlanV3 = (country: string) => {
	...
	const price = getEnv(`REBILL_${gateway ?? isDeprecateRebillMP}_${countryPrice}_V3_FREEMIUM`);
...
export const initRebillV3 = async (
	...
	const contactZoho: ContactCRM = await ssr.getEmailByIdZohoCRM('Contacts', email);
	...
	checkoutForm.on('approved', (event: RebillV3Event) => {
		...
		sendToZohoByRebillV3(event, user, country, product, setShow, setPaymentCorrect, setFaliedMessage);
	});
