...
export const CancelTrialButton: FC<{
	...
	return (
		<div className='ml-1 inline-block'>
			{text && `- ${text} `}
			<Link href='#' onClick={onClick} className='underline text-violet-custom hover:text-violet-custom'>
				{linkText}
			</Link>
		</div>
	);
...
export const ProductAccountButton: FC<{
	...
	const mappedStatus = state.statuses[product.product_code] ? state.statuses[product.product_code] : product.status;
