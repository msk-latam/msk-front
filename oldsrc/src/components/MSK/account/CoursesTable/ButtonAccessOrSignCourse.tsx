...
const ButtonAccessOrSignCourse: FC<ButtonAccessOrSignCourseProps> = ({ email, goToEnroll, goToLMS, item }) => {
	...
	const disabledRender = () => {
		return (
			<ButtonPrimary onClick={() => {}} sizeClass='py-0.5 sm:py-1 px-2 sm:px-5' disabled={true}>
				<span className='text-[14px] sm:text-sm font-bold'>Activar</span>
			</ButtonPrimary>
		);
	};

	if (statusOV.hasText === 'Preventa' || statusOV.hasText === 'Baja') {
			...
			{isDisabled ? (
				...
				<>
					<div className='flex gap-2 md:block'>
						<ButtonAccessCourse email={email} goToEnroll={goToEnroll} goToLMS={goToLMS} item={item} />
