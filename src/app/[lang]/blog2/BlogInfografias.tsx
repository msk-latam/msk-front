...
const BlogInfografias: React.FC = () => {
	...
	return (
		<section id='infografias' className='py-8 relative'>
			...
			<div className='relative -mx-4 sm:-mx-6 lg:mx-0 overflow-x-auto'>
				<div className='flex gap-6 whitespace-nowrap my-4'>
					{infografiaList.map((infografia, index) => {
						...
						return (
							<a key={index} href={link} className='relative bg-white shadow-md rounded-lg  block w-64 flex-shrink-0'>
								<div className='relative  h-48 '>
									<Image src={imageUrl} alt={alt} layout='fill' objectFit='cover' className='inset-0 rounded-t-lg ' />
								</div>
								<div className='absolute bottom-4  text-black px-3 py-1 rounded'>
									<h3 className='text-base font-semibold relative text-wrap'>{title}</h3>
								</div>
							</a>
						);
					})}
