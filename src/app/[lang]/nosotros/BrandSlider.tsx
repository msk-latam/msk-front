		...
		<div className='relative pb-5 overflow-visible md:overflow-hidden'>
			...
			>
				{brands.map((brand: any, index: number) => (
					<div key={index} className={`flex-shrink-0 group bg-[#F7F9FF] rounded-[30px] px-9 py-8 ${index === 0 ? '' : ''}`}>
						<a
							href={brand.url || brand.link}
							target='_blank'
							rel='noopener noreferrer'
							className='relative flex items-center justify-center h-20 cursor-pointer '
							onDragStart={preventImageDrag}
							onClick={handleClick}
						>
							<Image
								src={brand.imgDefault || brand.image}
								alt='Brand logo'
								width={brand.width || 200}
								height={100}
								className='object-contain transition-all duration-500 ease-in-out opacity-100 mix-blend-multiply '
							/>
						</a>
					</div>
				))}
			...
			<div className='flex justify-center gap-12 mt-4'>
				<button
					type='button'
					onClick={() => desktopScrollRef.current?.scrollBy({ left: -216, behavior: 'smooth' })}
					className='flex items-center justify-center w-10 h-10 bg-white border border-[#DBDDE2] rounded-full'
				>
					<ChevronLeft size={20} />
				</button>
				<button
