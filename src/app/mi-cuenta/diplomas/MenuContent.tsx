...
const MenuContent = forwardRef<HTMLDivElement, MenuContentProps>(({ style, closeMenu, course, user, badgeType }, ref) => {
	...
	const handleClickVer = () => {
		if (badgeUrl) {
			...
			const popupWindow = window.open('', '_blank', 'width=800,height=600,scrollbars=no,resizable=no');
	...
	const handleClickDownload = async () => {
		if (badgeUrl) {
			...
			try {
				...
				const fileExtension = badgeUrl.split('.').pop().toLowerCase();
				const fileName = `${badgeType}_${course.title.replace(/[^a-z0-9]/gi, '_')}.${fileExtension}`;
	...
	return (
		<div ref={ref} style={style} className='bg-white border rounded-2xl shadow-md z-[99] absolute'>
			<ul>
				<li className='flex items-center px-4 py-2 text-[#6474A6] hover:text-[#FF5D5E] rounded-t-2xl cursor-pointer'>
					...
					>
						<Image src={ver} height={16} width={16} alt='Ver' className='mr-2' />
						Ver
					</button>
				</li>
				<li className='flex items-center px-4 py-2 text-[#6474A6] hover:text-[#FF5D5E] cursor-pointer'>
					...
					>
						<Image src={descargar} height={16} width={16} alt='Descargar' className='mr-2' />
						Descargar
					</button>
				</li>
				<li className='flex items-center px-4 py-2 text-[#6474A6] hover:text-[#FF5D5E] rounded-b-2xl cursor-pointer'>
					...
					>
						<Image src={linkedin} height={16} width={16} alt='Compartir en LinkedIn' className='mr-2' />
