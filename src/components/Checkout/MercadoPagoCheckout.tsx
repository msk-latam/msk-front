...
const MercadoPagoCheckout: FC<MercadoPagoCheckoutProps> = ({ product, quotes, country, mountedInputObjectState }) => {
	...
	const createPaymentDetails = (profileCustomerInfo: any, contractDraftCreated: any) => ({
		trial: 1,
	...
	const handleSubmit = async (values: any) => {
		...
		try {
			...
			if (contract.status === 200 && contract.data) {
				...
				setErrorMessage('Hubo un problema al crear el contrato. Inténtalo de nuevo.');
				setStatusMessage('');
		...
		const paymentDetails = createPaymentDetails(profileCustomerInfo, contract.data);
	...
	useEffect(() => {
		...
		const checkAndSetState = () => {
			if (!sendRequestRef.current && typeof profile !== 'undefined' && profile != null && product?.ficha != null) {
	...
	const paymentStatusMessages = {
		approved: {
			...
			message: 'Revisa tu bandeja de entrada, spam o correos no deseados y sigue los pasos detallados.',
		...
		in_process: {
			...
			message: 'Tu pago está siendo procesado. Por favor, espera unos minutos y revisa tu correo para más información.',
		...
		error: {
			...
			message: 'Ocurrió un problema al procesar el pago. Por favor, intenta nuevamente o contacta a soporte.',
					...
					{paymentStatus ? (
						...
						>
							...
							<div className='absolute top-0 right-0 p-5 pt-5 '>
								<img src={paymentStatusMessages[paymentStatus]?.icon} alt='Status Icon' className='w-5 h-5' />
							</div>
							...
							<p className={`${paymentStatusMessages[paymentStatus]?.textColor} w-full font-normal`}>
								{paymentStatusMessages[paymentStatus]?.message}
							</p>

							<div className='mt-8'>
								{paymentStatus === 'error' ? (
									<ButtonPrimary href='#' onClick={() => window.location.reload()} className='w-full'>
										Volver a intentar
									</ButtonPrimary>
								) : (
									<div className='mt-8'>
						...
						<>
							...
							{/* Formulario de pago */}
							<div className={`mpc-field mb-5 ${onPaymentRequest ? 'mpc-field-requesting hidden' : ''}`}>
								<Formik initialValues={initialValues} validationSchema={cardMpValidation} onSubmit={handleSubmit}>
									{({ isSubmitting, isValid, dirty, setFieldValue }) => (
										<Form className='grid grid-rows-[1fr_auto] gap-3'>
											<div className={`${onPaymentRequest && 'hidden'} grid col-span-1 grid-cols-4 gap-3`}>
							...
							<div className='text-violet-wash flex items-center justify-center gap-x-3 mb-4'>
								...
								<Image src={mpImg.src} width={100} height={80} alt={'Mercado Pago Image'} />
							</div>
						</>
					)}
				</div>
			) : (
				<InputSkeleton className='w-[390px] mx-auto' text='Cargando datos de pago' />
			)}
		</>
	);
};
