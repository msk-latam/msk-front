...
const CategoryBadgeList: FC<CategoryBadgeListProps> = ({
	...
	const compararPorSlug = (a: any, b: any) => {
		...
		if (slugA?.includes('actualidad') && !slugB?.includes('actualidad')) {
			return 1;
		} else if (!slugA?.includes('actualidad') && slugB?.includes('actualidad')) {
			return -1;
	...
	const pillSwitch = ({ value, itemClass }: PillSwitchProps) => {
		switch (value) {
			case 'ebook':
				return (
					<Badge
						...
						color={'emerald-post'}
						href={country === '' ? `/tienda?recurso=guias-profesionales` : `/${country}/tienda?recurso=guias-profesionales`}
						icon='elearning'
					/>
				);
					...
					<>
						{sortedCategories.map((item, index) => (
							<Badge
								...
								href={
									country === ''
										? `/tienda?especialidad=${slugifySpecialty(item.name)}&recurso=curso`
										: `/${country}/tienda?especialidad=${slugifySpecialty(item.name)}&recurso=curso`
								}
								textSize={textSize}
							/>
						))}
					...
					<>
						{sortedCategoriespost.map((item, index) => (
							<Badge
								...
								href={
									country === ''
										? `/blog/archivo?categoria=${slugifySpecialty(item.name)}`
										: `/${country}/blog/archivo?categoria=${slugifySpecialty(item.name)}`
								}
								textSize={textSize}
							/>
						))}
			...
			case 'trial':
				return <Badge className={itemClass} name={categories[0]} color={'trial'} textSize={'text-[11px]'} />;
