...
const StoreContent: FC<{}> = () => {
	...
	const handlePageChange = (pageNumber: number) => {
		...
		if (!pageExists) {
			...
			if (products) {
				const currentItems = products.slice(indexOfFirstItem, indexOfFirstItem + itemsPerPage);
	...
	const applyFilters = () => {
		...
		const selectedSpecialties = storeFilters.specialties.map((filter: Specialty) => filter.name);
		const selectedProfessions = storeFilters.professions.map((filter: Profession) => filter.slug);
		const selectedResources = storeFilters.resources.map((filter: ResourceFilter) => filter.name);
		const selectedDurations = storeFilters.duration.map((filter: DurationFilter) => filter.slug);
		const selectedPage = storeFilters.page.map((filter: PageFilter) => filter.id);

		const selectedText =
			storeFilters.search && storeFilters.search.length > 2 ? storeFilters.search.pop() : storeFilters.search[0];
		...
		if (
			...
			let filterWithValueCourses = storeCourses.filter((c: WpProduct) => {
				if ((c.father_post_type === 'course' && Number(c.total_price) === 0) || c.is_test_product) {
					// console.log({ c });
					return false;
				}
			...
			const filteredProducts = storeCourses.filter((product: WpProduct) => {
				...
				const professionsMatch =
					...
					selectedProfessions.some((profession) =>
						prodProfessions.some((prodProfession) => prodProfession.toLowerCase().includes(profession?.toLowerCase())),
					);
				...
				if (searchText) {
					searchMatch = removeAccents(product.title.toLowerCase()).startsWith(removeAccents(searchText.toLowerCase()));
				}
        ...
        const environmentMatch =
          !product.is_test_product && product.father_post_type === 'course'; */
				...
				return specialtiesMatch && professionsMatch && resourcesMatch && durationsMatch && searchMatch;
			});
	...
	return (
		<section className=' course-content-area pb-90 animate-fade-down '>
			...
			{storeFilters.specialties.length > 0 && (
				<h1 className='text-xl sm:text-3xl mb-6'>Cursos de {storeFilters.specialties[0].name}</h1>
			)}
