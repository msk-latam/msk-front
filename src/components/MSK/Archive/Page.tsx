...
const PageArchiveComponent: FC<PageArchiveProps> = ({ className = '', posts }) => {
	...
	const [bannerImage, setBannerImage] = useState('/webp-images/banners/archivo.jpg');
	...
	const handleFilterChange = (e: { name: string }) => {
		...
		if (e.name === 'Más leídos') {
			const the_most_read = filteredPosts[0].the_most_read.map((post: { category: string }) => {
				return {
					...post,
					categories: [{ name: post.category, slug: post.category.toLowerCase() }],
				};
			});
		...
		useEffect(() => {
			...
			if (specialtyValue) {
				...
				filteredPosts = auxPosts.filter((post: PostDataType) => {
					return post.categories.some((category) => category.name.includes(notesJSON[categoryValue]));
				});
			}
			setCurrentPage(1);
			setAuxPosts(filteredPosts);
	...
	return (
		<>
			<div className={`nc-PageArchive pt-8 lg:pt-8 ${className} animate-fade-down `} data-nc-id='PageArchive'>
				{loadingPosts ? (
					<div className=''>
							...
							<header className='md:w-[120%] relative md:left-1/2 md:transform md:-translate-x-1/2  xl:max-w-screen-2xl '>
								<div className=' relative aspect-w-16 aspect-h-13 sm:aspect-h-9 lg:aspect-h-8 xl:aspect-h-5 rounded-3xl md:rounded-[40px] overflow-hidden z-0'>
									...
									<div className='absolute inset-0 bg-black text-white bg-opacity-30 flex flex-col items-center justify-center'>
										...
										<span className='block mt-4 text-neutral-300'>{[auxPosts.length]} Artículos</span>
									</div>
								</div>
							</header>
						) : null}
            ...
            >
              <span className="text-sm">Ver Especialidades</span>
								...
								{auxPosts.length ? (
									<>
										{currentItems.map((post, index) => (
											<Card11 key={`blog_${index}_${post.slug}`} post={post} className='rounded-xl' kind='blog' />
										))}
									</>
								) : (
					...
					<BlogSummary posts={posts} tabs={TABS_BLOG} loading={loadingPosts} className='py-16' heading='' desc='' />
				</div>
			</div>
			<NcModal
