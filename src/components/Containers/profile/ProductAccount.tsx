...
const ProductAccount: FC<Props> = ({ product, user, className, hoverEffect = false }) => {
	...
	const showHelp =
		product.ov === 'Baja' || product.ov === STATUS.SUSPEND || (isDisabled && !product.status?.includes(STATUS.TO_ENROLL));
				...
				<div className='course-cart'>
					<div className='course-info-wrapper'>
						<div className='cart-info-body'>
							<CategoryBadgeList categories={product.categories} color='yellow' isCourse={true} />
							<a onClick={handleClick}>
								<h3>{product.title}</h3>
							</a>
				...
				<div className='portfolio-course-2-content'>
					<div className='portfolio-course-wrapper'>
						<div>
							<div className='flex gap-2 flex-wrap'>
								{product.duration ? null : (
									<>
										<Badge icon='elearning' color='emerald-post' name='Guía profesional' textSize='text-[11px]' />
									</>
								)}
								<CategoryBadgeList categories={product.categories} color='yellow' isCourse={true} textSize='text-[11px]' />
							</div>
						...
						<div>
							...
							{showTip && <InfoText addClassNames='mt-2' text='¿No ves resultados? Intenta refrescar la pantalla.' />}
						</div>
					</div>
				</div>
				{product ? (
					<ProductAccountButton product={product} onRequest={onRequest} isRunning={isRunning} onClick={handleClick} />
				) : null}
			</div>
		</div>
	);
