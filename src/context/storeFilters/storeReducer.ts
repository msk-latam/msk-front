...
import { ResourceFilter, DurationFilter, Profession, Specialty, PageFilter } from '@/data/types';
...
export type Action =
	| {
			...
			payload: {
				...
				filterValue: Specialty | Profession | DurationFilter | ResourceFilter | PageFilter | string;
			};
	  }
	| {
			type: 'UPDATE_FILTER';
	...
	| {
			...
			payload: {
				filterType: keyof Filter;
				filterValue: Specialty | Profession | DurationFilter | ResourceFilter | PageFilter;
			};
	  }
	| {
			type: 'CLEAR_FILTERS';
				...
				return {
					...
					storeFilters: {
						...
						[action.payload.filterType]: state.storeFilters[action.payload.filterType].filter((filterValue: any) => {
							return filterValue !== action.payload.filterValue && filterValue.name !== action.payload.filterValue.name;
						}),
					},
				};
			}
		...
		case 'ADD_FILTER':
			...
			let stateAux = {
				...
				storeFilters: {
					...
					[action.payload.filterType]: [...state.storeFilters[action.payload.filterType], action.payload.filterValue],
				},
			};
