		...
		<section className='w-full font-raleway md:px-16 md:py-5 p-5'>
			...
			<div className='grid grid-cols-1 md:grid-cols-3 md:grid-rows-2 gap-6'>
				{postsToDisplay.map((post, index) => (
					<div key={post.id} className={`${index === 0 ? 'md:col-span-2 md:row-span-1' : 'md:col-span-1 md:row-span-1'}`}>
						<BlogCard
							title={post.title}
							subtitle={post.subtitle}
							author={post.author}
							date={post.date}
							readTime={post.readTime} // 👈 sin "min de lectura" acá
...
const EmptyState: React.FC<{
	...
	<section className='w-full '>
		<header className='mb-8'>
			...
			<p className='text-sm text-neutral-600'>Recursos para informarte y aprender de distintas maneras</p>
