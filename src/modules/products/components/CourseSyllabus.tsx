...
export default function CourseSyllabus({ slug, lang }: CourseSyllabusProps) {
	const [openIndex, setOpenIndex] = useState<number | null>(null);
	const [isModalOpen, setIsModalOpen] = useState(false);
	const { data, loading, error } = useCourseSyllabus(slug, lang);
	...
	return (
		<section className='w-full bg-white rounded-[38px] md:py-16 md:px-9 px-6 py-9 space-y-6'>
				...
				{data.modules.map((module, index) => (
					<div
						...
						className={`w-full h-auto px-6 py-4 flex flex-col justify-start items-start ${
							index !== data.modules.length - 1 ? 'border-b' : ''
						}`}
					>
						<button
						...
						>
							...
							<span>
								{openIndex === index ? <FaMinus className='text-[#29324f]' /> : <FaPlus className='text-[#29324f]' />}
							</span>
						</button>
			...
			{isModalOpen && <DownloadSyllabusModal slug={slug}  fileUrl={data.study_plan_file.url} onClose={handleCloseModal} isDownloadable={false} />}
		</section>
	);
}
