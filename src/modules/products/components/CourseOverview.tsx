...
const features = [
  {
    ...
    description:
      "Al finalizar y aprobar, descarga tu certificado y compártelo con colegas en LinkedIn.",
  },
  {
    ...
    return (
      ...
      <div className="flex flex-col md:flex-row md:flex-nowrap md:items-center md:justify-center md:gap-4 mb-10 text-sm text-[#29324f] w-full">
        {steps.map((item, idx) => (
          <div key={idx} className="flex items-start md:items-center gap-3 text-left">
            ...
            <span className="font-raleway font-bold text-base md:text-sm leading-tight pt-4 md:pt-0">
              {item.step}
            </span>
            {idx < steps.length - 1 && (
      ...
      <div className="grid gap-4 md:grid-cols-3 text-center">
        {features.map((item, index) => (
          <div
            key={index}
            className="rounded-xl border border-gray-200 py-6 px-2 shadow-sm bg-white"
          >
