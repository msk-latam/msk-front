...
const ViewOffer: React.FC<Props> = ({ isMobile = true }) => {
  ...
  return (
    <div className="bg-white rounded-t-3xl mt-4 h-full overflow-auto px-4">
      <div className="flex flex-row justify-center items-center px-6 py-8">
        <Link href="/main">
          <a className="absolute left-5 top-10 rounded-full border border-black p-2 text-gray-800">
            <ArrowLeft size={24} />
          </a>
        </Link>
      ...
      <div className="flex flex-col bg-gray-200 rounded-2xl">
        {data.offers.map((offer, index) => (
          <Link key={`offer-${index}`} href={offer.link.url}>
