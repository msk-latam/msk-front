// components/navbar/Navbar.tsx (versión simplificada)
...
const Navbar = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      setShowFloatingMenu(window.scrollY > 40);
    };
...
<section className="hidden md:flex md:flex-row items-center pt-2 mt-4">
  <div className="flex items-top max-w-6xl w-full mx-auto pl-2 pr-4 z-50 relative"> {/* 🔥 Añadido z-30 y relative */}
    {/* Logo */}
    <figure>
      <Image
        src="/images/msk-logo/logo.png"
    ...
    <nav
      className={`flex items-center flex-grow justify-between py-2 mx-16 px-5 transition-colors duration-300 ${
        isDiscoverOpen ? "bg-white rounded-t-3xl " : "bg-white shadow-md rounded-full"
      }`}
    >
      ...
      {/* 🔥 Dropdown desktop "embebido" visualmente debajo del navbar */}
  {isDiscoverOpen && (
