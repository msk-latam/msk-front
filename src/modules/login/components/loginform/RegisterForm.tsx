    ...
    <div className="w-full bg-white rounded-3xl shadow-md -mt-[40px] md:-mt-20 md:p-0 md:mb-20 z-[10] relative overflow-visible mx-auto">
      ...
      <section className="w-full max-w-[1632px] h-fit relative z-8 mx-auto px-6 pt-[84px] pb-28 md:py-16 md:px-9">
        <div className="text-center mb-6">
          <h1 className="md:text-[34px] text-2xl md:mb-6 mb-2 text-[#1A1A1A] font-medium font-raleway">
            Crear cuenta
          </h1>
        ...
        <form
          className="max-w-md mx-auto space-y-6 font-inter"
          onSubmit={(e) => {
            e.preventDefault();
            setSubmitted(true);
          }}
        >
          ...
          {['Google', 'Facebook', 'Apple'].map((provider) => (
            <button
              key={provider}
              className="w-full border border-gray-300 rounded-[38px] py-2 flex items-center justify-center gap-2 font-inter font-medium"
            >
              Crear con {provider}
              <img src={`/icons/${provider.toLowerCase()}.svg`} alt={provider} className="h-5 w-5" />
            </button>
          ))}
          ...
          <p className="text-center text-sm text-[#6E737C] mt-4 font-inter">
            ...
            <button
              type="button"
              onClick={onBack}
              className="text-[#9200AD] underline font-medium"
            >
              Inicia sesión aquí
            </button>
          </p>
