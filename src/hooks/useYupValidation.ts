...
const specialtyValidation = Yup.string().required('La especialidad es requerida');

const anotherProfessionValidation = Yup.string().test('other-profession', 'La profesion es requerida', function (value) {
...
const careerYearValidation = Yup.string().test('student-year', 'El año es requerido para estudiantes', function (value) {
	const profession = this.parent.Profesion;
...
const careerValidation = Yup.string().test('student-year', 'La carrera es requerido para estudiantes', function (value) {
	const profession = this.parent.Profesion;
	if (profession === 'Estudiante' && !value) {
...
const themesNewsletterValidation = Yup.array().of(Yup.string()).min(1, 'Se requiere al menos 1 tema de interés').required();

export const useYupValidation = () => {
	const contactFormValidation: Yup.Schema<ContactFormSchema> = Yup.object().shape({
	...
	const temarioFormValidation: Yup.Schema<TemarioFormSchema> = Yup.object().shape({
		First_Name: firstNameValidation,
		Last_Name: lastNameValidation,
		Email: emailValidation,
		Phone: phoneValidation,
	...
	const cancelSubscriptionValidation: Yup.Schema<CancelSubscriptionSchema> = Yup.object().shape({
		first_name: firstNameValidation,
		last_name: lastNameValidation,
		email: emailValidation,
	...
	const cardMpValidation = Yup.object().shape({
		cardNumber: Yup.string()
			...
			.matches(/^\d{16}$/, 'El número de tarjeta debe tener 16 dígitos y solo números'),

		expirationMonth: Yup.string()
			.required('El mes de expiración es obligatorio')
			.matches(/^(0[1-9]|1[0-2])$/, 'El mes de expiración debe ser un número entre 01 y 12'),

		expirationYear: Yup.string()
			.required('El año de expiración es obligatorio')
			.matches(/^\d{4}$/, 'El año de expiración debe tener 4 dígitos y solo números'),

		securityCode: Yup.string()
			.required('El código de seguridad es obligatorio')
			.matches(/^\d{3,4}$/, 'El código de seguridad debe tener 3 o 4 dígitos y solo números'),
	});
